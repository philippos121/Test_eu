"""
Integration tests v3: 5 fixture cases exercising the v3 scoring pipeline.

Each case tests that:
  - Evidence scoring behaves correctly
  - Willingness from behavioural facts is correct
  - Blend weighting produces the right direction of result
...
class TestCase1_StrongCase:
    ...
    def test_provability_high_with_full_evidence(self):
        ...
        p_ev = evidence_to_prob(ev.total)  # ~0.982 for score=100
        stat = bayes_update(4, 6, 1, 1)   # 1 win from events
        p_provable = blend(p_ev, stat.mean, PROVABLE_EV_WEIGHT)
        assert p_provable > 0.6

    def test_high_p_cash_when_llm_rates_valid(self):
        """Simulate a strong case end-to-end with good LLM output."""
        llm_valid = 0.85   # LLM sees valid claim
        stat_valid = bayes_update(6, 2, 1, 1).mean   # 1 win from events
        p_valid = blend(llm_valid, stat_valid, LEGAL_LLM_WEIGHT)

        ev = score_evidence(self.case, self.facts)
        p_ev = evidence_to_prob(ev.total)
        stat_provable = bayes_update(4, 6, 1, 1).mean
        p_provable = blend(p_ev, stat_provable, PROVABLE_EV_WEIGHT)

        llm_ability = 0.80   # active company, no insolvency
        p_will = score_willingness(self.facts)
        p_individual = 0.5 * llm_ability + 0.5 * p_will
        stat_payment = bayes_update(5, 5, 0, 0).mean
        p_payment = blend(p_individual, stat_payment, PAYMENT_IND_WEIGHT)

        p_cash = _compute_p_cash(p_valid, p_provable, p_payment)
        assert p_cash > 0.40   # strong case should have high probability
...
class TestCase2_WeakCase:
    ...
    def test_low_p_cash_with_weak_evidence(self):
        ev = score_evidence(self.case, self.facts)
        p_ev = evidence_to_prob(ev.total)         # ~0.018 for score≈15

        llm_valid = 0.40   # LLM uncertain: oral agreement, limited info
        stat_valid = bayes_update(6, 2, 0, 0).mean
        p_valid = blend(llm_valid, stat_valid, LEGAL_LLM_WEIGHT)

        stat_provable = bayes_update(4, 6, 0, 0).mean
        p_provable = blend(p_ev, stat_provable, PROVABLE_EV_WEIGHT)

        p_individual = 0.5 * 0.50 + 0.5 * score_willingness(self.facts)
        stat_payment = bayes_update(5, 5, 0, 0).mean
        p_payment = blend(p_individual, stat_payment, PAYMENT_IND_WEIGHT)

        p_cash = _compute_p_cash(p_valid, p_provable, p_payment)
        assert p_cash < 0.30   # weak case
...
class TestCase3_DisputedCase:
    ...
    def test_dispute_reduces_llm_validity(self):
        ...
        llm_valid_disputed = 0.42   # quality dispute reduces entstanden
        llm_valid_clean    = 0.80
        stat_valid = bayes_update(6, 2, 0, 0).mean

        p_valid_disputed = blend(llm_valid_disputed, stat_valid, LEGAL_LLM_WEIGHT)
        p_valid_clean    = blend(llm_valid_clean,    stat_valid, LEGAL_LLM_WEIGHT)
        assert p_valid_disputed < p_valid_clean

    def test_respondent_affects_provable_bayes(self):
        """defendant_responded means no default judgment — only contested wins count."""
        s, n = count_events(self.events, "provable")
        assert s == 0 and n == 0   # DEFENDANT_RESPONDED not in provable mapping
...
class TestCase4_InsolvencyCase:
    ...
    def test_insolvency_kills_p_payment(self):
        ...
        llm_ability_insolvent = 0.05   # insolvency found → 5/100
        p_will = score_willingness(self.facts)

        p_individual = 0.5 * (llm_ability_insolvent) + 0.5 * p_will
        stat_payment = bayes_update(5, 5, 0, 0).mean
        p_payment = blend(p_individual, stat_payment, PAYMENT_IND_WEIGHT)
        assert p_payment < 0.35   # severely reduced
    ...
    def test_p_cash_low_despite_valid_claim(self):
        ...
        p_cash = _compute_p_cash(p_valid, p_provable, p_payment)
        # Good validity and provability but near-zero payment
        assert p_cash < 0.25
    ...
    def test_insolvency_driver_appears(self):
        ...
        posteriors = {
            "valid":    bayes_update(6, 2, 0, 0),
            "provable": bayes_update(4, 6, 0, 0),
            "payment":  bayes_update(5, 5, 0, 0),
        }
        drivers = compute_drivers(ev, legal, ability, 0.5, posteriors)
...
class TestCase5_RepeatDefendant:
    ...
    def setup_method(self):
        ...
        # Events from prior cases with this defendant
        self.events = [
            _make_event(CaseEventType.JUDGMENT_WIN),
            _make_event(CaseEventType.COLLECTION_FAILED),
        ]
    ...
    def test_low_willingness_reduces_payment(self):
        """Low willingness pulls the individual component down."""
        llm_ability = 0.65   # company is active but evasive
        p_will = score_willingness(self.facts)   # ~0.15
        p_individual = 0.5 * llm_ability + 0.5 * p_will   # ~0.40

        stat_payment = bayes_update(5, 5, 0, 1).mean   # 1 collection failure
        p_payment = blend(p_individual, stat_payment, PAYMENT_IND_WEIGHT)
        assert p_payment < 0.50

    def test_payment_rate_lower_after_collection_failure(self):
        s, n = count_events(self.events, "payment")
        assert s == 0
        assert n == 1   # 1 failure
        post = bayes_update(5, 5, s, n)
        prior_mean = bayes_update(5, 5, 0, 0).mean
        assert post.mean < prior_mean   # failure pushes mean down
    ...
    def test_negative_willingness_driver(self):
        ...
        posteriors = {
            "valid":    bayes_update(6, 2, 1, 1),
            "provable": bayes_update(4, 6, 1, 1),
            "payment":  bayes_update(5, 5, 0, 1),
        }
        p_will = score_willingness(self.facts)
        drivers = compute_drivers(ev, legal, ability, p_will, posteriors)
